                    ...
                    <div class="nav-dropdown-menu" id="new-chat-menu" style="width: 260px; max-height: 350px; overflow-y: auto; padding: 0.5rem;">
                        ...
                        <?php if(!empty($network_peers)): ?>
                            <?php foreach($network_peers as $peer): 
                                $peer_course = $peer['alumni_course'] ?? $peer['student_course'] ?? '';
                                $peer_member_id = get_student_id_string($peer['id'], $peer_course);
                            ?>
                                <a href="#" class="dropdown-item select-peer-item" 
                                   data-id="<?php echo $peer['id']; ?>" 
                                   data-name="<?php echo htmlspecialchars($peer['name']); ?>"
                                   data-role="<?php echo htmlspecialchars($peer['role']); ?>"
                                   data-member-id="<?php echo htmlspecialchars($peer_member_id); ?>">
                                    <div style="display: flex; flex-direction: column; line-height: 1.2;">
                                        <strong><?php echo htmlspecialchars($peer['name']); ?></strong>
                                        <span style="font-size: 0.68rem; opacity: 0.6;"><?php echo $peer_member_id; ?></span>
                                    </div>
                                    <span style="font-size: 0.65rem; color: var(--theme-accent-blue); margin-left: auto; text-transform: uppercase;"><?php echo $peer['role']; ?></span>
                                </a>
                            <?php endforeach; ?>
                        <?php else: ?>
